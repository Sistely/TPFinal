...
namespace Desktop_TP_Final
{
    public partial class FormGerenciar : Form
    {
        ...
        {
            DeleteUsuario(int.Parse(listUsuarios.GetItemText(listUsuarios.SelectedItem).Split(',')[0]));
        }

        private void editar_btn_Click(object sender, EventArgs e)
        {
  
            GetUsuarioById(int.Parse(listUsuarios.GetItemText(listUsuarios.SelectedItem).Split(',')[0]));
        }
            ...
            using (var client = new HttpClient())
            {
                using (var response = await client.GetAsync(URI))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        listUsuarios.DataSource = JsonConvert.DeserializeObject<Usuario[]>(ProdutoJsonString).ToList();
                    }
                    else
                    {
